...
const UserDetailsPage = ({ user, onClose, onGroupCreated, sentimentStats = {} }) => {
  ...
  return (
    <>
      ...
      >
        <div
          className="max-w-lg w-full max-h-[90vh] rounded-2xl shadow-2xl overflow-y-auto"
          style={{ 
        ...
        >
          {/* Header */}
          <div className="flex justify-between items-center p-4 border-b border-gray-200 sticky top-0 bg-white/90 backdrop-blur-sm">
            <h2 className="text-lg font-bold text-gray-800">
              {user.isGroup ? "Group Details" : "Profile"}
            </h2>
            <button
              onClick={onClose}
            ...
            <div className="text-center">
              <div className="relative inline-block mb-3">
                <img
                  ...
                  alt="User"
                  className="w-20 h-20 rounded-full object-cover shadow-lg mx-auto ring-2 ring-blue-100"
                />
                {user.isGroup && (
                ...
                {user.isGroup && user.members && (
                  <p className="text-sm text-gray-500 mb-4">{user.members.length} members</p>
                )}
              </div>
            {/* Shared Interests - Simplified */}
            {!user.isGroup && matchedInterests.length > 0 && (
              <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                <h4 className="text-sm font-semibold text-green-800 mb-2 text-center">
                  🤝 Shared Interests ({matchedInterests.length})
                </h4>
